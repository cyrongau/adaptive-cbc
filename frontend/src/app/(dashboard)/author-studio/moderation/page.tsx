...
const ACCENT = {
  super_admin: { border: 'border-amber-400', text: 'text-amber-300', bg: 'bg-amber-500', hover: 'hover:bg-amber-600', light: 'bg-amber-900/20', textLight: 'text-amber-400' },
  institution_admin: { border: 'border-[#47a263]', text: 'text-[#7eda95]', bg: 'bg-[#47a263]', hover: 'hover:bg-[#47a263]/80', light: 'bg-[#47a263]/20', textLight: 'text-[#7eda95]' },
  default: { border: 'border-blue-500', text: 'text-blue-600', bg: 'bg-blue-500', hover: 'hover:bg-blue-600', light: 'bg-blue-50', textLight: 'text-blue-600' },
};

export default function ModerationPage() {
  const { user } = useAuthStore();
  const accent = ACCENT[user?.role as keyof typeof ACCENT] || ACCENT.default;
  ...
  const [statusTab, setStatusTab] = useState('for-review');
  ...
  const fetchQueue = useCallback(async () => {
    ...
    try {
      ...
      if (statusTab === 'approved') params.set('status', 'approved');
      else if (statusTab === 'rejected') params.set('status', 'rejected');
      else if (filterStatus) params.set('status', filterStatus);
      params.set('page', String(page));
      params.set('limit', '20');

      const res = await api.get(`/questions/moderation/queue?${params}`, { timeout: 15000 });
  ...
  }, [searchTerm, filterSubject, filterGrade, filterStatus, statusTab, page]);
  ...
  const handleScoreQuality = async (q: QuestionSummary) => {
    ...
    try {
      ...
      console.error('Quality score error:', err?.response?.data || err);
      toast.error(err?.response?.data?.message || 'Quality scoring failed');
    } finally {
      setScoring(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-1 mb-6 border-b border-gray-200 dark:border-gray-700">
          {[
            ...
            <button
              key={tab.key}
              onClick={() => { setStatusTab(tab.key); setPage(1); setSelected(null); setQualityScore(null); }}
              ...
              <>
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                  ...
                  <div className="prose prose-sm max-w-none mb-4">
                    <HtmlContent html={selected.content} className="text-gray-900 dark:text-white" />
                  </div>
                  ...
                  {selected.explanation && (
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                      ...
                      <HtmlContent html={selected.explanation} className="text-sm text-gray-600 dark:text-gray-400" />
                    </div>
                  )}
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        ...
                        disabled={scoring}
                        className={`text-xs px-3 py-1.5 ${accent.light} ${accent.text} rounded-lg ${accent.text.startsWith('text-amber') ? 'hover:bg-amber-900/40 dark:hover:bg-amber-900/50' : 'hover:bg-[#47a263]/30 dark:hover:bg-[#47a263]/40'} transition-colors flex items-center gap-1`}
                      >
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                    ...
                    {statusTab === 'for-review' ? (
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <button
                            ...
                            disabled={actionLoading !== null}
                            className="flex items-center justify-center gap-1.5 px-3 py-2 bg-red-500 hover:bg-red-600 disabled:bg-red-400 text-white rounded-lg text-sm font-medium transition-colors"
                          >
                      ...
                      <p className="text-xs text-gray-500 text-center py-3">
                        This question has been {statusTab}. Use the "For Review" tab to take action.
                      </p>
                    )}
                  </div>
                </div>
